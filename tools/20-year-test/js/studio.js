...
(function (global) {
  ...
  var prefs = {
    ...
    mode: 'one',
    /* Where the script comes from: 'comparison' generates it from the
       Setup tab; 'own' is a script you paste, with nothing from the
       calculator in it. Every studio, the prompter and the camera work
       the same either way. */
    source: 'comparison',
  ...
  try {
    ...
    if (saved) {
      ...
      if (p.source === 'own') { prefs.source = 'own'; }
  ...
  /* =====================================================================
     YOUR OWN SCRIPT
     A script pasted in, sectioned by headings, with nothing from the
     calculator in it. Each section is a slide with the same pictures,
     camera and like-subscribe controls as a generated one.
     ===================================================================== */
  var OWN_KEY = 'bcb-20-year-test-v1-own';
  var own = { title: '', text: '' };
  try {
    var savedOwn = localStorage.getItem(OWN_KEY);
    if (savedOwn) { var o = JSON.parse(savedOwn); if (o && typeof o === 'object') { Object.assign(own, o); } }
  } catch (e) { /* fine */ }
  function saveOwn() {
    try { localStorage.setItem(OWN_KEY, JSON.stringify(own)); } catch (e) { /* fine */ }
  }

  /* A line starting with # opens a section and names it. A line in
     [square brackets], or starting with >, is an instruction to yourself:
     it goes to the prompter, never to the screen, and it costs no time.
     With no # lines at all, a blank line starts the next section. */
  function parseOwn(text) {
    var lines = String(text || '').replace(/\r/g, '').split('\n');
    var hasHeads = lines.some(function (l) { return /^#/.test(l.trim()); });
    var sections = [], cur = null;
    function openSection(title) { cur = { title: title, items: [] }; sections.push(cur); }
    lines.forEach(function (raw) {
      var l = raw.trim();
      if (hasHeads) {
        if (/^#/.test(l)) { openSection(l.replace(/^#+\s*/, '') || 'Untitled'); return; }
        if (!l) { return; }
        if (!cur) { openSection('Section 1'); }
      } else {
        if (!l) { if (cur && cur.items.length) { cur = null; } return; }
        if (!cur) { openSection('Section ' + (sections.length + 1)); }
      }
      if (/^\[.*\]$/.test(l) || /^>/.test(l)) {
        cur.items.push({ note: l.replace(/^\[|\]$/g, '').replace(/^>\s*/, '').trim() });
        return;
      }
      N.sentences(l).forEach(function (sentence) { cur.items.push({ say: sentence }); });
    });
    return sections.filter(function (sec) { return sec.items.length; }).map(function (sec, i) {
      var spoken = sec.items.filter(function (it) { return it.say; }).map(function (it) { return it.say; });
      return {
        id: 'own-' + (i + 1), kind: 'own', title: sec.title, kicker: '',
        lines: spoken, items: sec.items,
        seconds: spoken.length ? N.beatSeconds(spoken) : 8
      };
    });
  }
  function ownScript() {
    var beats = parseOwn(own.text);
    return {
      beats: beats,
      totalSeconds: beats.reduce(function (t, x) { return t + x.seconds; }, 0),
      titles: [], thumbnails: [], seed: 0, own: true
    };
  }
  function ownMode() { return prefs.source === 'own'; }
  ...
  function build() {
    if (ownMode()) {
      script = ownScript();
      if (idx >= script.beats.length) { idx = Math.max(0, script.beats.length - 1); }
      return script;
    }
  ...
  function visualFor(b) {
    ...
    switch (b.kind) {
      ...
      case 'own':
        /* The title is already on the slide; the pictures come through
           the inset, the feature or the main layouts. Nothing else, so
           nothing from the calculator can leak onto a pasted script. */
        return wrap;
  ...
  function openPresentation() {
    if (!build() || !script.beats.length) { return; }
    ...
    var head =
      '<div class="st-top">' +
        '<div class="st-brand">Blue Collar Business<span>' +
          (ownMode() ? esc(own.title || 'My episode') : 'The ' + BCB.app.getLast().sim.cfg.years + '-Year Test') + '</span></div>' +
    ...
    function poke() {
      ...
      hideTimer = setTimeout(function () { if (overlay) { overlay.classList.add('idle'); } }, 2600);
    }
  ...
  var ONE_INSET = { setup: 1, education: 1, business: 1, dependency: 1, hours: 1, freedom: 1,
    invest: 1, earlymoney: 1, disclaimer: 1, outro: 1, title: 1, own: 1 };
  ...
  function paintBackground() {
    ...
    if (!asset && b.kind === 'own') {
      /* A pasted script has no careers to draw on: the brand library
         plays behind it, rotating so the episode is not one picture. */
      var brandKeys = ['title', 'intro', 'outro'];
      asset = BCB.media.assetFor(brandKeys[idx % 3], idx % 3 === 1 ? 'clip' : 'still') ||
              BCB.media.assetFor('title', 'still') || BCB.media.assetFor('intro', 'clip') || null;
    } else if (!asset) {
  ...
  /* With a pasted script there are no careers to have pictures of; the
     brand slots still play behind the slides, and the timing rules still
     govern each section's list. */
  function ownPicturesMarkup() {
    var M = BCB.media;
    var slots = [['brand', 'intro', 'Behind the slides'], ['brand', 'title', 'Behind the slides (2)'], ['brand', 'outro', 'Behind the slides (3)']];
    return '<h3>Pictures and clips</h3>' +
      '<p class="sub">Each section takes its own pictures in the script below. These three rotate behind any section ' +
      'that has none of its own, or that keeps its own pictures in front.</p>' +
      '<div class="pic-grid">' +
      slots.map(function (sl) {
        var e = M.libraryEntry(sl[0], sl[1]);
        var box = function (field, ownV, has, accept, label) {
          var f = M.fileInfo(ownV) || (/^file:/.test(ownV) ? { name: 'uploaded file', size: 0 } : null);
          var id = sl[0] + '|' + esc(sl[1]) + '|' + field;
          return '<div class="field"><label>' + label + '</label>' +
            '<div class="pic-row">' +
            (f
              ? '<span class="pic-file" title="' + esc(f.name) + '">' + esc(f.name || 'uploaded file') + ' \u00b7 ' + fmtBytes(f.size) + '</span>' +
                '<button type="button" class="btn btn-o btn-sm" data-picclear="' + id + '">Remove</button>'
              : '<input type="url" data-pic="' + id + '" value="' + esc(ownV) + '" placeholder="' +
                (has && !ownV ? 'using the library ' + (field === 'clip' ? 'clip' : 'picture') : 'paste a link') + '">') +
            '<label class="btn btn-o btn-sm pic-upload">Choose file<input type="file" accept="' + accept + '" data-picfile="' + id + '" hidden></label>' +
            '</div></div>';
        };
        return '<div class="pic-slot">' +
      ...
      '<div class="field"><label for="holdPic">Hold each picture</label>' +
      '<input type="number" id="holdPic" min="2" max="30" step="1" value="' + prefs.media.imageHold + '">' +
      '<div class="hint">Seconds before the next one takes over.</div></div>' +
      '<div class="field"><label for="holdClip">Hold each clip</label>' +
      '<input type="number" id="holdClip" min="3" max="60" step="1" value="' + prefs.media.clipHold + '">' +
      '<div class="hint">Seconds.</div></div>' +
      '<div class="field"><label for="fillInset">Margin when a picture fills the frame</label>' +
      '<input type="number" id="fillInset" min="0" max="20" step="1" value="' + prefs.media.fillInset + '">' +
      '<div class="hint">% in from the edge.</div></div>' +
      '</div>';
  }
  ...
  function prompterMarkup() {
    return '<!DOCTYPE html><html><head><meta charset="utf-8"><title>Prompter - The 20-Year Test</title>' +
      ...
      'p.past{color:#3a3d43}' +
      '.note{margin:0 0 .55em;padding:.2em .6em;border-left:3px solid var(--accent);color:#b8905e;font-size:.62em;' +
        'font-style:italic;line-height:1.35}' +
      '.note::before{content:"note  ";font-style:normal;font-size:.8em;letter-spacing:.12em;text-transform:uppercase;color:var(--accent)}' +
  ...
  function renderPrompter() {
    ...
    /* A pasted script carries instructions between its sentences; they
       render as notes, which the pacing (which counts <p>) never sees. */
    var items = b.items || b.lines.map(function (l) { return { say: l }; });
    var li = 0;
    col.innerHTML = '<div class="beat">' + esc(b.kicker || b.title) + '</div>' +
      items.map(function (it) {
        return it.note ? '<div class="note">' + esc(it.note) + '</div>'
                       : '<p data-i="' + (li++) + '">' + esc(it.say) + '</p>';
      }).join('');
    scrollPrompter(true);
  }
  ...
  /* =====================================================================
     THE STUDIO TAB
     ===================================================================== */
  function ownSectionsMarkup() {
    if (!script.beats.length) {
      return '<p class="hint" style="color:var(--muted)">Paste a script above and its sections appear here, each with ' +
        'its own pictures, camera choice and like-subscribe button.</p>';
    }
    return script.beats.map(function (b, i) {
      return '<details class="fieldset"' + (i === 0 ? ' open' : '') + '><summary>' + (i + 1) + '. ' + esc(b.title) +
        ...
        b.items.map(function (it) {
          return it.note ? '<p class="own-note">' + esc(it.note) + '</p>' : '<p>' + esc(it.say) + '</p>';
        }).join('') + '</div>' +
  ...
  function ownTiles() {
    ...
    return '<div class="tile"><div class="k">Sections</div><div class="v">' + script.beats.length + '</div></div>' +
      '<div class="tile"><div class="k">Estimated runtime</div><div class="v">' + mins + 'm ' + secs + 's</div>' +
        '<div class="n">at about 155 words a minute, instructions not counted</div></div>' +
      '<div class="tile"><div class="k">Episode</div><div class="v" style="font-size:1.05rem">' +
        esc(own.title || 'My episode') + '</div></div>';
  }
  ...
  function renderStudio() {
    ...
    host.innerHTML =
      ...
      '<div class="st-sources">' +
        [['comparison', 'This comparison', 'The script, the charts and every number come from the Setup tab.'],
         ['own', 'My own script', 'Paste a script and add pictures to each section. Nothing from the calculator.']]
        .map(function (m) {
          return '<button class="st-mode' + (prefs.source === m[0] ? ' on' : '') + '" data-source="' + m[0] + '">' +
            '<span class="k">' + esc(m[1]) + '</span><span class="d">' + esc(m[2]) + '</span></button>';
        }).join('') +
      '</div>' +
      '<div class="tiles" id="stTiles" style="margin-bottom:16px">' +
      (isOwn ? ownTiles() :
      ...
      '<div class="field-inline" style="flex-wrap:wrap;gap:8px">' +
        '<button class="btn" data-st="stage" id="stStartBtn"' + (isOwn && !script.beats.length ? ' disabled' : '') + '>Start presenting</button>' +
      ...
      '<div class="card" id="picCard">' + (isOwn ? ownPicturesMarkup() : picturesMarkup()) + '</div>' +

      (isOwn
      ? '<div class="card"><h2>My own script</h2>' +
        '<p class="sub">Nothing on these slides comes from the comparison. Paste the script; every section becomes a slide ' +
        'with its own pictures, camera choice and like-subscribe button below. Saved in this browser as you type.</p>' +
        '<div class="field"><label for="ownTitle">Episode title</label>' +
        '<input type="text" id="ownTitle" value="' + esc(own.title) + '" placeholder="Shows on the brand plate at the top of every slide"></div>' +
        '<div class="field"><label for="ownText">Script</label>' +
        '<textarea id="ownText" rows="18" spellcheck="true" style="width:100%;font:inherit;font-size:.92rem;line-height:1.5;padding:10px;' +
        'border:1px solid var(--line);border-radius:5px;background:var(--surface-1);color:var(--text)" ' +
        'placeholder="# Cold open\nToday we are looking at the one number nobody tells you about.\n[show the truck picture here]\n\n# The setup\nHere is how the test works...">' + esc(own.text) + '</textarea>' +
        '<div class="hint">A line starting with <strong>#</strong> begins a new section and is its title. The sentences under it are what you say. ' +
        'A line in <strong>[square brackets]</strong>, or starting with <strong>&gt;</strong>, is an instruction to yourself: it shows in the prompter, ' +
        'it is never on screen, and it is not counted in the time. With no # lines at all, a blank line starts the next section. ' +
        'Pictures stay with the section number, so add sections at the end rather than in the middle once you have placed them.</div></div>' +
        '<div id="ownSections">' + ownSectionsMarkup() + '</div>' +
  ...
  function onStudioEvent(ev) {
    var t = ev.target;
    var srcBtn = t.closest && t.closest('[data-source]');
    if (srcBtn && ev.type === 'click') {
      prefs.source = srcBtn.dataset.source === 'own' ? 'own' : 'comparison';
      savePrefs();
      script = null; idx = 0;
      renderStudio(); renderPrompter();
      if (overlay) { build(); renderStage(); }
      return;
    }
    if (t.id === 'ownTitle' && ev.type === 'input') {
      own.title = t.value; saveOwn();
      var ep = document.querySelector('#stTiles .tile:last-child .v');
      if (ep) { ep.textContent = own.title || 'My episode'; }
      if (overlay) { var bs = overlay.querySelector('.st-brand span'); if (bs) { bs.textContent = own.title || 'My episode'; } }
      return;
    }
    if (t.id === 'ownText' && ev.type === 'input') {
      own.text = t.value; saveOwn();
      /* Re-parse a beat after the typing pauses; only the section list
         redraws, so the cursor stays where it is. */
      clearTimeout(ownTimer);
      ownTimer = setTimeout(function () {
        var keep = script && script.beats[idx] ? script.beats[idx].id : null;
        script = ownScript();
        var at = keep ? script.beats.map(function (b) { return b.id; }).indexOf(keep) : -1;
        idx = at >= 0 ? at : Math.max(0, Math.min(idx, script.beats.length - 1));
        var secs = document.getElementById('ownSections');
        if (secs) { secs.innerHTML = ownSectionsMarkup(); }
        var tiles = document.getElementById('stTiles');
        if (tiles) { tiles.innerHTML = ownTiles(); }
        var sb = document.getElementById('stStartBtn');
        if (sb) { sb.disabled = !script.beats.length; }
        renderPrompter();
        if (overlay) { if (script.beats.length) { renderStage(); } else { closePresentation(); } }
      }, 250);
      return;
    }
  ...
  function init() {
    ...
    if (BCB.recorder) {
      ...
      BCB.recorder.setNameSource(function () {
        ...
        if (ownMode()) { return slug(own.title || 'my-episode') || 'my-episode'; }
        var L = BCB.app.getLast();
        if (!L) { return ''; }
